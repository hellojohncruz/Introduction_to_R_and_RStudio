...
```{r plot-girls-vs-year-arbuthnot}
...
```{r plot-girls-vs-year-line-arbuthnot}
ggplot(data = arbuthnot, aes(x = year, y = girls)) + 
  geom_line()
```
...
```{r plot-girls-vs-year-line-lm_model}
ggplot(data = arbuthnot, aes(x = year, y = girls)) + 
  geom_point() + 
  geom_smooth(method = 'lm', se = FALSE)
```

Overall we can see a moderately positive trend in the number of girls baptized between 1629-1710. However, there was a strong negative trend between 1640-1660 that requires further investigation as to its possible cause. Added was a linear model line to show said positive trend, however, a better fitted model for time-series analysis would be more appropriate. 
...
```{r calc-total-bapt-vars-arbuthnot}
...
```{r calc-total-bapt-numbers-1710-arbuthnot}
tail(arbuthnot$boys, 1) + tail(arbuthnot$girls, 1)
```
Verifying last index in data set. Both equals 14,928
...
```{r calc-total-bapt-vars-save-arbuthnot}
arbuthnot <- arbuthnot %>%
...
```{r plot-total-vs-year-arbuthnot}
...
```{r calc-prop-boys-to-girls-vars-arbuthnot}
arbuthnot <- arbuthnot %>%
...
```{r calc-prop-boys-vars-arbuthnot}
arbuthnot <- arbuthnot %>%
...
```{r plot-boy_ratio-vs-year-arbuthnot}
ggplot(data = arbuthnot, aes(x = year, y = boy_ratio)) + 
  geom_point() + geom_smooth(method = 'lm', se = FALSE)
```

We see that the proportion of boys over time has a weak negative trend throughout the years. However, the proportion of boys is generally above 51%. 
...
```{r boys-more-than-girls-arbuthnot}
arbuthnot <- arbuthnot %>%
...
```{r plot-count-more_boys-vs-year-arbuthnot}
ggplot(data = arbuthnot, aes(more_boys)) + 
  geom_bar()
```

Surprisingly, there was never a year accounted for where there were more girls born than boys. This is further validated based on the previous graph showing boy_ratio never falling under 50%
...
```{r summarize min and max years}
present %>%
  summarize(min = min(year), max = max(year))
```
The data includes birth records in the United States from 1940 to 2002. 

```{r dimensions of present data frame}
dim(present)
```
The dimensions for the present data frame is 63 rows and 3 columns. 

```{r columns of present data frame}
colnames(present)
```
The column names within this data set are year, boys, girls.

5.  How do these counts compare to Arbuthnot's? Are they of a similar magnitude?

```{r compare average values of boys and girls between the two data sets}
present <- present %>%
  mutate(total = boys + girls)

arbuthnot_avg_total <- mean(arbuthnot$total)
present_avg_total <- mean(present$total)

present_avg_total / arbuthnot_avg_total
```

The average counts over the years from Arbuthnot's and the present data sets are vastly different. On average, the present data is over 320 times more children compared to Arbuthnot's. It is also important to note that Arbuthnot's data range of 82 years compared to the present data of 63 years is almost two decades more, yet on average, still significantly less children. 
...
``` {r plots: boy_ratio over time; count-more_boys-vs-year}
ggplot(data = present, aes(x = year, y = boy_ratio)) + 
  geom_point() + geom_smooth(method = 'lm', se = FALSE)

ggplot(data = present, aes(more_boys)) + 
  geom_bar()

```

Similarly, both proportions of boys born over time have a weak negative trend and is generally above 51%. Strikingly, our present data bar plot shows that each year also had more boys born than girls just like Arbuthnot's. 
...
The most total number of births in the US was in 1961 at 4,268,326
